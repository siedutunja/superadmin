<template>
  <div>
    <b-row v-if="listaCalificaciones.length">
      <b-col>
        <h4 class="text-danger">¡Lo sentimos! {{listaCalificaciones.length}}</h4><br>
        <h5>
          En el momento no pude continuar al paso de Priorización de Factores Críticos, ya que aún se encuentran algunos componentes sin Valorar. <br>
          A continuación se detallan los componenetes sin Valorar:
        </h5>
        <vue-good-table :columns="encabColumnas" :rows="listaCalificaciones" styleClass="vgt-table condensed bordered striped" :line-numbers="true">
        </vue-good-table>
      </b-col>
    </b-row>
    <b-row v-else>
        ...
        <b-card class="encabezadoVista">
          <b-row>
            <b-col lg="12" class="mt-2 text-center">
              ...
              <h4>PRIORIZACIÓN DE FACTORES CRÍTICOS</h4>
      ...
      <b-row v-if="listaComponentes.length">
        <b-col lg="12" md="12">
          <div v-for="item in listaComponentes" :key="item.id">
            <ComponenteCritico :datosComponente="{id: item.id, componente: item.componente}" />
          </div>
        </b-col>
      </b-row>
...
<script>
  ...
  import ComponenteCritico from '@/views/calidad/ComponenteCritico'
    ...
    data () {
      return {
        ...
        encabColumnas: [
          ...
          { label: 'Valoración', field: 'rubrica', sortable: false }
        ]
      }
    },
    methods: {
